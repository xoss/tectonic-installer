data "scw_region" "current" {
  current = true
}

resource "scw_s3_bucket" "tectonic" {
  ...
  bucket = "${var.tectonic_scw_assets_s3_bucket_name == "" ? format("%s%s-%s", "a", var.tectonic_cluster_name, md5(format("%s-%s", data.scw_region.current.name , var.tectonic_base_domain))) : var.tectonic_scw_assets_s3_bucket_name }"
...
resource "scw_s3_bucket_object" "tectonic_assets" {
  bucket = "${scw_s3_bucket.tectonic.bucket}"
  key    = "assets.zip"
...
# kubeconfig
resource "scw_s3_bucket_object" "kubeconfig" {
  bucket  = "${scw_s3_bucket.tectonic.bucket}"
  key     = "kubeconfig"
