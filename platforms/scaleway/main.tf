provider "scw" {
  region  = "${var.tectonic_scw_region}"
  profile = "${var.tectonic_scw_profile}"
  version = "1.7.0"
}

data "scw_availability_zones" "azs" {}
...
module "vpc" {
  ...
  cidr_block               = "${var.tectonic_scw_vpc_cidr_block}"
  ...
  external_master_subnets  = "${compact(var.tectonic_scw_external_master_subnet_ids)}"
  external_vpc_id          = "${var.tectonic_scw_external_vpc_id}"
  external_worker_subnets  = "${compact(var.tectonic_scw_external_worker_subnet_ids)}"
  extra_tags               = "${var.tectonic_scw_extra_tags}"
  private_master_endpoints = "${var.tectonic_scw_private_endpoints}"
  public_master_endpoints  = "${var.tectonic_scw_public_endpoints}"
  ...
  # "tectonic_scw_master_custom_subnets" and "tectonic_scw_worker_custom_subnets" variables.
  #
  # To enable mode B, make sure that "tectonic_scw_master_custom_subnets" and "tectonic_scw_worker_custom_subnets"
  # ARE NOT SET.

  # These counts could be deducted by length(keys(var.tectonic_scw_master_custom_subnets))
  # but there is a restriction on passing computed values as counts. This approach works around that.
  master_az_count = "${length(keys(var.tectonic_scw_master_custom_subnets)) > 0 ? "${length(keys(var.tectonic_scw_master_custom_subnets))}" : "${length(data.scw_availability_zones.azs.names)}"}"
  worker_az_count = "${length(keys(var.tectonic_scw_worker_custom_subnets)) > 0 ? "${length(keys(var.tectonic_scw_worker_custom_subnets))}" : "${length(data.scw_availability_zones.azs.names)}"}"
  ...
  master_subnets = "${concat(values(var.tectonic_scw_master_custom_subnets),list("padding"))}"
  worker_subnets = "${concat(values(var.tectonic_scw_worker_custom_subnets),list("padding"))}"
  # The split() / join() trick works around the limitation of ternary operator expressions
  # only being able to return strings.
  master_azs = "${ split("|", "${length(keys(var.tectonic_scw_master_custom_subnets))}" > 0 ?
    join("|", keys(var.tectonic_scw_master_custom_subnets)) :
    join("|", data.scw_availability_zones.azs.names)
  )}"
  worker_azs = "${ split("|", "${length(keys(var.tectonic_scw_worker_custom_subnets))}" > 0 ?
    join("|", keys(var.tectonic_scw_worker_custom_subnets)) :
    join("|", data.scw_availability_zones.azs.names)
  )}"
}

module "etcd" {
  source = "../../modules/scw/etcd"
  ...
  ec2_ami                    = "${var.tectonic_scw_ec2_ami_override}"
  ec2_type                   = "${var.tectonic_scw_etcd_ec2_type}"
  etcd_iam_role              = "${var.tectonic_scw_etcd_iam_role_name}"
  ...
  root_volume_iops           = "${var.tectonic_scw_etcd_root_volume_iops}"
  root_volume_size           = "${var.tectonic_scw_etcd_root_volume_size}"
  root_volume_type           = "${var.tectonic_scw_etcd_root_volume_type}"
  s3_bucket                  = "${scw_s3_bucket.tectonic.bucket}"
  sg_ids                     = "${concat(var.tectonic_scw_etcd_extra_sg_ids, list(module.vpc.etcd_sg_id))}"
  ssh_key                    = "${var.tectonic_scw_ssh_key}"
...
module "ignition_masters" {
  ...
  assets_location           = "${scw_s3_bucket_object.tectonic_assets.bucket}/${scw_s3_bucket_object.tectonic_assets.key}"
  base_domain               = "${var.tectonic_base_domain}"
  ...
  kubeconfig_fetch_cmd      = "/opt/s3-puller.sh ${scw_s3_bucket_object.kubeconfig.bucket}/${scw_s3_bucket_object.kubeconfig.key} /etc/kubernetes/kubeconfig"
...
module "masters" {
  source = "../../modules/scw/master-asg"

  assets_s3_location                   = "${scw_s3_bucket_object.tectonic_assets.bucket}/${scw_s3_bucket_object.tectonic_assets.key}"
  autoscaling_group_extra_tags         = "${var.tectonic_autoscaling_group_extra_tags}"
  scw_lbs                              = "${module.vpc.scw_lbs}"
  base_domain                          = "${var.tectonic_base_domain}"
  cluster_id                           = "${module.tectonic.cluster_id}"
  cluster_name                         = "${var.tectonic_cluster_name}"
  container_images                     = "${var.tectonic_container_images}"
  ...
  container_linux_version              = "${module.container_linux.version}"
  ec2_ami                              = "${var.tectonic_scw_ec2_ami_override}"
  ec2_type                             = "${var.tectonic_scw_master_ec2_type}"
  extra_tags                           = "${var.tectonic_scw_extra_tags}"
  ...
  master_iam_role                      = "${var.tectonic_scw_master_iam_role_name}"
  master_sg_ids                        = "${concat(var.tectonic_scw_master_extra_sg_ids, list(module.vpc.master_sg_id))}"
  private_endpoints                    = "${var.tectonic_scw_private_endpoints}"
  public_endpoints                     = "${var.tectonic_scw_public_endpoints}"
  root_volume_iops                     = "${var.tectonic_scw_master_root_volume_iops}"
  root_volume_size                     = "${var.tectonic_scw_master_root_volume_size}"
  root_volume_type                     = "${var.tectonic_scw_master_root_volume_type}"
  s3_bucket                            = "${scw_s3_bucket.tectonic.bucket}"
  ssh_key                              = "${var.tectonic_scw_ssh_key}"
...
module "workers" {
  ...
  container_linux_version              = "${module.container_linux.version}"
  ec2_ami                              = "${var.tectonic_scw_ec2_ami_override}"
  ec2_type                             = "${var.tectonic_scw_worker_ec2_type}"
  extra_tags                           = "${var.tectonic_scw_extra_tags}"
  ign_ca_cert_id_list                  = "${module.ignition_masters.ca_cert_id_list}"
  ...
  load_balancers                       = "${var.tectonic_scw_worker_load_balancers}"
  root_volume_iops                     = "${var.tectonic_scw_worker_root_volume_iops}"
  root_volume_size                     = "${var.tectonic_scw_worker_root_volume_size}"
  root_volume_type                     = "${var.tectonic_scw_worker_root_volume_type}"
  s3_bucket                            = "${scw_s3_bucket.tectonic.bucket}"
  sg_ids                               = "${concat(var.tectonic_scw_worker_extra_sg_ids, list(module.vpc.worker_sg_id))}"
  ssh_key                              = "${var.tectonic_scw_ssh_key}"
  subnet_ids                           = "${module.vpc.worker_subnet_ids}"
  vpc_id                               = "${module.vpc.vpc_id}"
  worker_iam_role                      = "${var.tectonic_scw_worker_iam_role_name}"
}

module "dns" {
  source = "../../modules/dns/route53"

  api_external_elb_dns_name      = "${module.vpc.scw_api_external_dns_name}"
  api_external_elb_zone_id       = "${module.vpc.scw_elb_api_external_zone_id}"
  api_internal_elb_dns_name      = "${module.vpc.scw_api_internal_dns_name}"
  api_internal_elb_zone_id       = "${module.vpc.scw_elb_api_internal_zone_id}"
  api_ip_addresses               = "${module.vpc.scw_lbs}"
  base_domain                    = "${var.tectonic_base_domain}"
  cluster_id                     = "${module.tectonic.cluster_id}"
  cluster_name                   = "${var.tectonic_cluster_name}"
  console_elb_dns_name           = "${module.vpc.scw_console_dns_name}"
  console_elb_zone_id            = "${module.vpc.scw_elb_console_zone_id}"
  custom_dns_name                = "${var.tectonic_dns_name}"
  ...
  tectonic_external_private_zone = "${var.tectonic_scw_external_private_zone}"
  tectonic_external_vpc_id       = "${module.vpc.vpc_id}"
  tectonic_extra_tags            = "${var.tectonic_scw_extra_tags}"
  tectonic_private_endpoints     = "${var.tectonic_scw_private_endpoints}"
  tectonic_public_endpoints      = "${var.tectonic_scw_public_endpoints}"
  tectonic_vanilla_k8s           = "${var.tectonic_vanilla_k8s}"
}
